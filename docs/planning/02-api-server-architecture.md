...
**Phase 1 measurement baseline.** In BCD 8.0.13, the compact raw `javascript.builtins.Array` subtree is 54,217 bytes and contains 51 `__compat` blocks. The pinned fixture asserts both measurements. This is a source-data baseline, not a generated normalized-payload measurement and not a size cap; Phase 3 still records compact, gzip, Brotli, parse, and memory costs for the emitted artifact before deciding whether any cap is warranted.
